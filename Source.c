...
char buffer[30];
int k, l;
int highscores[5]; //array to store top 5 high scores
char names[5][30]; //array to store top 5 player names
...
int main(void)
{
	...
	for (;;)
	{
		if (menu == 0) { 
			...
			for (;;) {
				...
				if (game_ended == 1) {
					...
					for (int w = 0; w < 5; w++) {
						...
						else if (highscores[w] != NULL) {
							if (highscores[w] > diff) {
								int x = 4 - w;
								while (x > 0) {
									highscores[w + x] = highscores[w + x - 1];
									x--;
								}
								highscores[w] = diff;
								printf("Please enter your name: ");
			...
			printf("Highscores: \n\n");
			int y = 0;
			if (y < 5 && highscores[y] != NULL) {
				printf("%d. %s %d seconds and %d milliseconds\n", y+1, names[y], highscores[y] / 1000, highscores[y] % 1000);
				getchar();
			}
			else if (highscores[0] == NULL) {
				printf("No highscores have been recorded.");
				getchar();
			}
			getchar();
			menu = 0;
