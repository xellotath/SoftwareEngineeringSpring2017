...
int k, l;
int encryption_switch;
int decryption_switch;
int highscores[6] = { 300000,360000,420000,480000,560000, 1000000 }; //array to store top 5 high scores
char names[6][30] = { {"b0b"},{"bobdy"},{"bodber"},{"dobbet"},{"bed"},{"blud"} }; //array to store top 5 player names
int stored = 0; //variable to determine if a highscore has been stored yet this round, incremented to one when highscore has been stored
int temp;
char tempn[30];
	...
	for (;;)
	{
		if (menu == 0) {
			...
			for (;;) {
				...
				clock_t start = clock(), diff; //storing the starting time to be compared to time when finished
				...
				if (game_ended == 1) {
					diff = clock() - start; //difference in time from starting time til now
											//putting time into milli seconds
					printf("Enter your name: ");
					scanf("%s", names[5]);
					highscores[5] = diff;
					for (int w = 1; w <= 5; w++) {
						for (int z = 5; z >= w; z--) {
							if (highscores[z - 1] > highscores[z]) {
								temp = highscores[z - 1];
								highscores[z - 1] = highscores[z];
								highscores[z] = temp;
								strcpy(tempn, names[z - 1]);
								strcpy(names[z - 1], names[z]);
								strcpy(names[z], tempn);
							}
						}
			...
			if (replay_selection != -1) {
				...
				if (fgets(buffer, 30, replayfile) != NULL) {

					decryption_switch = atoi(buffer) -2017;
			...
			printf("Highscores: \n\n");
			for (int y = 0; y < 5; y++) {
				printf("%d. %s CLEARED THIS GAME IN %d.%d seconds\n", y + 1, names[y], highscores[y] / 1000, highscores[y] % 1000);
...
void write_replay() {
	...
	fprintf(replayfile, "%d\n", encryption_switch +2017);

	for (int k = 0; k < sc; k++) {
		for (int i = 0; i < 25; i++)
		{
			strcpy(string, replay_map[k][i]);
			encrypt(string);
			fprintf(replayfile, "%s", string);
		}
	}
